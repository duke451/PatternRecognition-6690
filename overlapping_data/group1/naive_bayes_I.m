...
sigma_square = (trace(cov1) + trace(cov2) + trace(cov3) + trace(cov4))./8;
common_cov = [sigma_square 0; 0 sigma_square];
...
for p = 1:size(total_test,1)
    ...
    for j = 1:size(x,2)
        first_term = 1./sqrt(2*pi*common_cov(j,j));
        second_term = -0.5*(x(1,j)-mean4(1,j))^2 ./(common_cov(j,j));
...
for xvalue = -6:0.1:10
    for yvalue = -15:0.1:15
        ...
        for j = 1:size(point,2)
            first_term = 1./sqrt(2*pi*common_cov(j,j));
            second_term = -0.5*(point(1,j)-mean4(1,j))^2 ./(common_cov(j,j));
            pxy4 = pxy4*(first_term)*exp(second_term);
        end
        pxy4 = pxy4*(size(train4,1)./size(total_train,1));
